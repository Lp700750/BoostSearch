...
namespace Lp700_search
{
  ...
  public:
    ...
    {
      //获取单例对象
      _index=index::Index::getInstance();

      //根据index来建立索引
      _index->builtIndex(input);
    }
    ...
    void Search(const std::string& query,std::string* json_string)
    {
      ...
      for(std::string word:words)
      {
        ...
        //对搜到的词进行搜索index
        index::invertList* invertlist=_index->invertSearch;
      ...
      {
        index::Docfo* doc=_index->farwardSearch(iter,docId);
        if(doc==nullptr)
        {
